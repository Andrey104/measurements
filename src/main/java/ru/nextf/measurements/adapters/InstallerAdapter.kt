...
class InstallerAdapter (notesList: List<Installers>) : RecyclerView.Adapter<RecyclerView.ViewHolder>() {
    ...
    override fun onCreateViewHolder(viewGroup: ViewGroup, i: Int): InstallerAdapter.ViewHolder? {
        var v = LayoutInflater.from(viewGroup.context).inflate(ru.nextf.measurements.R.layout.list_item_installer, viewGroup, false)
        return ViewHolder(v)
    }
    ...
    class ViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        ...
        init {
            textViewNameInstaller = itemView.findViewById(ru.nextf.measurements.R.id.textViewNameInstaller)
        }
    }
